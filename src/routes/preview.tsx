  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Wardogs Chat — StreamElements Chat Overlay" },
      {
        ...
        content:
          "Ammo-crate chat panels, clear Twitch role tags and combat dispatches for subs, bits, raids and more. Paste-in install, deep customization.",
      },
...
type FireKind = "chat" | "firstchat" | "subscriber" | "cheer" | "raid" | "follower" | "tip";
...
const TRIGGERS: Trigger[] = [
  ...
  {
    label: "New recruit (sub)",
    kind: "subscriber",
    payload: { name: "PVT_HALLOWAY", amount: 1, tier: "1000" },
  },
  {
    label: "Re-enlist (resub)",
    kind: "subscriber",
    payload: { name: "GHOST_RECON88", amount: 14, tier: "2000" },
  },
  {
  ...
  {
    label: "Tracer round (50 bits)",
    kind: "cheer",
    payload: { name: "TRENCHRAT", amount: 50, message: "keep firing" },
  },
  {
    label: "Mortar strike (2,500 bits)",
    kind: "cheer",
    payload: { name: "DELTA_MIKE", amount: 2500, message: "danger close" },
  },
  {
    label: "Airstrike (10,000 bits)",
    kind: "cheer",
    payload: { name: "K9Handler", amount: 10000, message: "fire mission out" },
  },
  {
    label: "Incoming convoy (raid)",
    kind: "raid",
    payload: { name: "WARHOUND_ACTUAL", amount: 312 },
  },
...
const STEPS = [
  ...
  "Click Done. Every option now appears in the widget's settings panel: colors, size, role tags, per-event toggles and sounds.",
];
    ...
    <main className="min-h-screen">
      ...
      <header className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto flex max-w-5xl flex-col items-center px-6 py-16 text-center md:py-24">
          <div className="wardogs-mark" aria-label="Wardogs K9 unit">
            <span className="wardogs-mark-kicker">K9 // STREAM UNIT</span>
            <span className="wardogs-mark-main">
              <b>WAR</b>
              <i>///</i>
              <b>DOGS</b>
            </span>
            <span className="wardogs-mark-footer">
              <span>EST. 2026</span>
              <span>LOYALTY · DUTY · HONOR</span>
            </span>
          </div>
          <p className="stencil mt-6 text-xs text-primary md:text-sm">
            Twitch chat overlay · StreamElements
          </p>
          ...
          <p className="mt-5 max-w-xl text-lg text-muted-foreground md:text-xl">
            Ammo-crate chat panels, clear Twitch role tags and combat dispatches for every sub, bit
      ...
      <section id="files" className="mx-auto max-w-6xl px-6 py-14" aria-labelledby="files-heading">
        ...
        <div className="mt-6 crate rivets">
          ...
          <pre className="max-h-[420px] overflow-auto bg-background/60 p-5 text-sm leading-relaxed">
            <code className="font-mono text-muted-foreground">{sources[active] ?? "Loading…"}</code>
          </pre>
        </div>
      </section>
